...
public class IccInstrumentDestination 
{
    ...
    public SootMethod generateOnBindMethod(SootClass sc)
    {
        ...
        Unit getIntentU = Jimple.v().newIdentityStmt(                 
    ...
    public void instrumentDummyMainMethod(SootClass compSootClass, SootField intentSootField)
    {
    	...
    	for (Iterator<Unit> iter = units.snapshotIterator(); iter.hasNext(); )
    	{
    		...
    		//Transfer Intent for such components that take an Intent as a parameter and do not leverage getIntent() method for retrieving the received Intent.
    		...
    		for (int i = 0; i < types.size(); i++)
    		{
    			...
    			if (type.equals(MESSAGE_TYPE))
    			{
    				...
    				{
    					System.out.println("Assign Message for " + stmt.getInvokeExpr().getMethod() + " fails.");
    				}
    			}
    	...
    	for (Iterator<Unit> iter = units.snapshotIterator(); iter.hasNext(); )
    	{
			...
			if (! method.isStatic())
			{
	    		...
	    		else
	    		{
	    	 		...
	    	 		Unit setMsgU = Jimple.v().newAssignStmt(     
	    	 				msgV,
		    	...
		    	for (Iterator<Unit> iter = units.snapshotIterator(); iter.hasNext(); )
		    	{
		    		...
		    		if (stmt.containsInvokeExpr())
		    		{
		    			...
		    			if (toBeCheckedMethod.getName().equals("send") && 
		    					equalsOrSubclassOf(toBeCheckedMethod.getDeclaringClass(), Scene.v().getSootClass("android.os.Messenger")))
		    			{
